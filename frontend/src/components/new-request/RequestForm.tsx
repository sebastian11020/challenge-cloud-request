...
export function RequestForm({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white/90 p-5 shadow-md">
      {error && (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <select
              value={requestTypeId}
              disabled={loading}
              onChange={(e) => onChangeType(e.target.value ? Number(e.target.value) : '')}
              className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-100"
              required
            >
              <option value="">
