    ...
    <section className="space-y-3">
      ...
      <div className="flex items-center justify-between text-[11px] text-slate-500">
        ...
        {hasResults && (
          <div className="flex items-center gap-2">
            <span>
              Página <span className="font-semibold text-slate-700">{currentPage}</span>{' '}
              de {totalPages}
            </span>
          </div>
        )}
        ...
        <table className="min-w-full text-sm">
          <thead className="border-b border-slate-100 bg-slate-50 text-xs text-slate-500">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-medium">Estado anterior</th>
          ...
          <tbody>
            {pageEvents.length === 0 && !loading && !error && (
              <tr>
                <td colSpan={8} className="px-3 py-5 text-center text-xs text-slate-500">
              ...
              >
                <td className="px-3 py-2 align-top">
                  <button
                    ...
                    className="font-mono text-xs text-sky-700 underline underline-offset-2 hover:text-sky-900"
                  >
                    #{event.requestId}
                  </button>
                </td>
                ...
                <td className="px-3 py-2 align-top text-xs">
                  <div className="flex flex-col">
                    <span className="font-medium text-slate-800">{event.actor}</span>
                ...
                <td
                  className="max-w-xs truncate px-3 py-2 align-top text-xs text-slate-600"
                  title={event.comment ?? ''}
                >
                  {event.comment ?? '—'}
                </td>
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-500">
          ...
          <div className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-1 py-0.5">
            <button
              ...
              className="rounded-full px-2 py-1 text-xs hover:bg-slate-50 disabled:opacity-40"
            >
              ← Anterior
            </button>
