  ...
  useEffect(() => {
    ...
    const fetchRequests = async () => {
      try {
        ...
        console.error(err);
        setError(
          err?.message ?? 'No se pudieron cargar tus solicitudes. Intenta nuevamente.'
        );
      } finally {
        setLoading(false);
      }
    ...
    <div>
      ...
      <header className="mb-4 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center rounded-full bg-slate-900 px-3 py-1.5 text-xs font-medium text-white shadow-sm transition-colors hover:bg-slate-800"
          >
            Nueva solicitud
          </button>
        </div>
      ...
      {!loading && !error && requests.length === 0 && (
        <div className="rounded-xl border border-dashed border-slate-300 bg-white/60 px-4 py-6 text-center">
          <p className="text-sm text-slate-600">Aún no has creado solicitudes.</p>
          <p className="mt-1 text-xs text-slate-500">
            Puedes iniciar una nueva solicitud para que entre al flujo de aprobación.
          </p>
          <button
            type="button"
            onClick={handleCreateNew}
            className="mt-3 inline-flex items-center rounded-full bg-slate-900 px-3 py-1.5 text-xs font-medium text-white shadow-sm transition-colors hover:bg-slate-800"
          >
