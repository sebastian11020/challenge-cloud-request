...
export default function DetailRequest() {
  ...
  const canDecide =
    !!request && request.status === 'PENDIENTE' && (isResponsable || isAdmin);
  ...
  const handleStatusChange = async (target: 'approve' | 'reject') => {
    ...
    try {
      ...
      setDecisionComment('');
      toast.success(
        `Solicitud ${target === 'approve' ? 'aprobada' : 'rechazada'} correctamente `
      );
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div>
      <button
        type="button"
        onClick={goBack}
        className="mb-3 inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-medium text-slate-600 shadow-sm transition-colors hover:border-slate-300 hover:bg-slate-50"
      >
        <ArrowLeft size={14} className="text-slate-500" />
        <span>Volver</span>
      </button>

      {loading && <p className="text-sm text-slate-500">Cargando solicitud...</p>}
