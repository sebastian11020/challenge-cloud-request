...
export default function ApproverRequestsPage() {
  ...
  const { pending, history, loading, error } = useApproverRequests(currentUser.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Bandeja de aprobación</h1>
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/60 px-3 py-2 text-xs text-slate-500">
          <span>Bandeja de entrada</span>
          <span>
            {loading ? 'Cargando...' : `${pending.length} solicitud(es) pendiente(s)`}
          </span>
        </div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/60 px-3 py-2 text-xs text-slate-500">
          ...
          <span>
            {loading ? 'Cargando...' : `${history.length} solicitud(es) procesadas`}
          </span>
        </div>
      ...
      {loading && (
        <p className="text-sm text-slate-500">Cargando solicitudes asignadas...</p>
      )}
    </div>
  );
}
